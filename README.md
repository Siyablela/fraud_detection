...
Query API :8000
	│
	│ GET /api/v1/transactions/{audit_id}
	▼
Client
```
...
```json
{
	"correlation_id": "external-ref-1001",
	"user_id": "user-42",
	"amount": 12500,
	"category": "RETAIL"
}
...
`correlation_id` is optional and can be provided by the caller for cross-system traceability.

`timestamp` is optional and is generated automatically when omitted.

The system always generates an `audit_id` for each call. That `audit_id` is persisted and returned to the caller.

For provenance, callers can optionally pass these headers:

- `x-request-id`: end-to-end request identifier.
- `x-actor-id`: caller identity (service or user principal).
- `x-actor-type`: actor class (for example `service`, `user`, `batch`).
...
Capture the `audit_id` from the producer response and use it for retrieval:
...
$auditId = $queued.audit_id

Invoke-RestMethod `
		"http://127.0.0.1:8000/api/v1/transactions/$auditId" | `
...
- `audit_id`: generated by the system for each processed call.
- `correlation_id`: optional caller-supplied identifier echoed back.
- `ruleset_hash`: deterministic fingerprint of the rules used for that decision.
- `is_fraud`: boolean decision.
- `triggered_rules`: matching rule IDs.
- `persisted`: whether the result was written to PostgreSQL.

Stored transaction records include `created_date` and `updated_date`.

Retrieve the append-only audit trail for a transaction:

```powershell
Invoke-RestMethod `
		"http://127.0.0.1:8000/api/v1/transactions/$auditId/audit?limit=100" | `
		ConvertTo-Json -Depth 8
```

Audit events are tamper-evident via hash chaining (`prev_hash`, `event_hash`) seeded by `AUDIT_HASH_SECRET`.
...
| `AUDIT_HASH_SECRET` | Secret used to key audit hash chain | `change-me-to-a-long-random-secret` |

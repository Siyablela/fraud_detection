...
MIGRATE_TRANSACTION_ID_TO_CORRELATION_ID = """
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        ALTER TABLE transactions RENAME COLUMN transaction_id TO correlation_id;
    END IF;
END $$;
"""
...
async def create_pool() -> asyncpg.Pool:
    ...
    async with pool.acquire() as connection:
        await connection.execute(MIGRATE_TRANSACTION_ID_TO_CORRELATION_ID)
...
async def save_transaction(pool: asyncpg.Pool, result: dict[str, Any]) -> None:
    await pool.execute(
        ...
        INSERT INTO transactions (
            correlation_id, user_id, amount, category, timestamp,
...
async def get_transaction(pool: asyncpg.Pool, correlation_id: str) -> dict[str, Any] | None:
    row = await pool.fetchrow(
        """
        SELECT correlation_id, user_id, amount, category, timestamp,
               is_fraud, triggered_rules
        FROM transactions
